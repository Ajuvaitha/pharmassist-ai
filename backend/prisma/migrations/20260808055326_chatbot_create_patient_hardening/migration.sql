...
BEGIN
  ...
  BEGIN
    PERFORM p_dob::date;
    PERFORM p_admission_date::date;
  EXCEPTION WHEN others THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Invalid date (use YYYY-MM-DD with a real calendar date)');
  END;
  ...
  EXCEPTION WHEN unique_violation THEN
    RETURN jsonb_build_object('ok', false, 'error', 'Could not allocate a patient MRN, please retry');
  END;
