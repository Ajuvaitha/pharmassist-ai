...
function generateResponse(input: string): string {
  ...
  if (q.includes('warfarin') && (q.includes('aspirin') || q.includes('interaction'))) {
    return `**Warfarin + Aspirin — Significant Interaction**

Concurrent use of Warfarin and Aspirin substantially increases bleeding risk through two independent mechanisms:

1. **Pharmacodynamic synergy** — Both are anticoagulants; Aspirin irreversibly inhibits platelet COX-1, while Warfarin inhibits vitamin K–dependent clotting factors (II, VII, IX, X).
2. **Pharmacokinetic displacement** — High-dose Aspirin can displace Warfarin from plasma protein binding, raising free drug concentration and INR.

**Clinical guidance:**
- Avoid combination unless specifically indicated (e.g. mechanical heart valves, high-risk ACS).
- If co-prescribed, monitor INR closely and watch for signs of bleeding — GI bleeds are most common.
- Preferred alternative: consider a PPI if GI prophylaxis is needed.

Flag this combination for prescriber review before dispensing.`;
  }
...
**Standard protocol:**
- Start at 500mg once or twice daily with meals to minimise GI side effects.
- Titrate by 500mg/week to target dose (usually 1000–2000mg/day in divided doses).
- Always administer **with or just after food**.

**Key contraindications to check before dispensing:**
- eGFR < 30 mL/min/1.73m² (hold or reduce dose at eGFR 30–45)
- Planned IV contrast procedure within 48 hours — hold Metformin and restart 48h post-procedure
- Active hepatic impairment
- History of lactic acidosis

**Patient counselling points:**
- Mild nausea, diarrhoea, or metallic taste common early on — usually resolves within 2 weeks.
- Does not cause hypoglycaemia when used as monotherapy.
- Ensure adequate hydration.`;
  }

  if (q.includes('reorder') || q.includes('stock') || q.includes('inventory')) {
    return `**Reorder Level Protocol — Korle Bu Teaching Hospital**

**Trigger thresholds:**
- 🔴 **Critical** — Stock at or below 20% of par level. Immediate emergency order required. Alert pharmacy supervisor.
- 🟡 **Low** — Stock at 20–40% of par level. Raise standard purchase order within 24 hours.
- 🟢 **Adequate** — Stock above 40% of par level. No action needed.

**Reorder process:**
1. Pharmacist identifies low/critical item in Inventory module.
2. Raise a Purchase Order (PO) referencing the drug's standard supplier.
3. Supervisor reviews and approves orders above GH₵ 500.
4. On delivery, verify batch number, expiry date, and quantity before accepting.
5. Log restock event in the system — this appears in Recent Activity.

**Controlled drugs** follow a separate procurement register with dual-pharmacist sign-off.`;
  }

  if (q.includes('tds') || q.includes('qds') || q.includes('bd') || q.includes('od') || (q.includes('dosing') && q.includes('frequency'))) {
    return `**Dosing Frequency Reference**

| Code | Meaning | Times per day | Default schedule |
|------|---------|---------------|-----------------|
| **OD** | Once daily | 1 | Morning |
| **BD** | Twice daily | 2 | Morning + Evening |
| **TDS** | Three times daily | 3 | Morning + Afternoon + Evening |
| **QDS** | Four times daily | 4 | Morning + Afternoon + Evening + Night |
| **PRN** | As needed | Variable | Patient-triggered |
| **STAT** | Immediately, once | 1 (urgent) | Now |

**Clinical notes:**
- Space TDS doses 6–8 hours apart for consistent plasma levels.
- QDS is often 6-hourly in acute settings; clarify with prescriber if not specified.
- Some drugs (e.g. antibiotics) have strict interval requirements — confirm on the prescription.`;
  }

  if (q.includes('antibiotic') || q.includes('dispensing') || q.includes('before dispensing')) {
    return `**Pre-Dispensing Checklist — Antibiotics**

Before dispensing any antibiotic, verify:

✅ **Allergy screen** — Check patient record for penicillin, cephalosporin, or sulphonamide allergy. Cross-reactivity is clinically significant.

✅ **Culture & sensitivity** — Is the antibiotic empirical or culture-directed? Broad-spectrum agents should ideally be de-escalated once sensitivities are known.

✅ **Renal/hepatic function** — Many antibiotics require dose adjustment (e.g. Gentamicin, Vancomycin, Meropenem at low eGFR).

✅ **Interaction check** — Common pairings to flag: Metronidazole + Alcohol, Fluoroquinolones + Antacids, Macrolides + QT-prolonging agents.

✅ **Duration** — Confirm number of days. Over-prescribing drives resistance. Escalate if course exceeds 14 days without documented rationale.

✅ **Route appropriate** — IV antibiotics should be stepped down to oral equivalent as soon as the patient can tolerate it (IV-to-oral switch programme).`;
  }

  if (q.includes('stop order') || q.includes('stop prescription')) {
    return `**Stop Order Protocol**

A Stop Order discontinues an active prescription. The following steps apply:

1. **Verify authority** — Stop orders must be issued by a registered prescriber (doctor). Pharmacists and nurses cannot initiate a stop order independently.

2. **Document reason** — Select from approved reasons:
   - Treatment complete
   - Adverse reaction
   - Toxicity suspected
...
3. **Controlled drugs** — Require dual-pharmacist sign-off and must be entered into the controlled drug register with the stop reason and prescriber signature.

4. **Nursing notification** — The ward nurse must be notified before the next scheduled dose to prevent inadvertent administration.

5. **Audit trail** — The stop order is logged in Recent Activity with timestamp, drug name, patient, and reason.

If a prescriber requests a verbal stop order, document it as a verbal order and follow up with written confirmation within 24 hours.`;
  }

  if (q.includes('interaction') || q.includes('drug interaction')) {
    return `**Drug Interaction Screening**

Common high-risk interactions to flag at dispensing:

| Drug A | Drug B | Risk | Action |
|--------|--------|------|--------|
| Warfarin | NSAIDs | ↑ Bleeding | Avoid or monitor INR |
| ACE Inhibitor | Potassium-sparing diuretic | Hyperkalaemia | Monitor K⁺ |
| Methotrexate | NSAIDs | MTX toxicity | Contraindicated |
| Digoxin | Amiodarone | Digoxin toxicity | Halve Digoxin dose |
| Simvastatin | Amiodarone | Myopathy/rhabdomyolysis | Limit Simvastatin ≤20mg |
| SSRIs | Tramadol | Serotonin syndrome | Avoid; use alternative analgesic |
| Fluoroquinolones | Antacids | ↓ Absorption | Separate by 2–4 hours |

For a specific drug pair, ask me directly. I can also check patient-specific combinations if you share the prescription details.`;
  }

  if (q.includes('patient') && (q.includes('allerg') || q.includes('contraindication'))) {
    return `**Allergy & Contraindication Checks**

Before dispensing to any patient:

1. **Review allergy record** — Check the patient chart for documented drug, food, or excipient allergies. Allergy badge appears in red on the patient card.

2. **Cross-reactivity** — Be alert to structural cross-reactivity:
   - Penicillin → Cephalosporins (~1–2% cross-react; lower with later generations)
   - Sulphonamide antibiotics → other sulphonamide-containing drugs (thiazides, furosemide, celecoxib)
   - Codeine → other opioids (variable)

3. **Excipient allergies** — Some patients react to lactose, gelatin capsules, peanut oil (in some oral drops), or dyes. Check product SPC if patient has unusual reactions.

4. **Document any new reactions** — Any adverse reaction observed post-dispensing must be documented in the patient record and reported via the hospital's pharmacovigilance system.

Would you like information on a specific allergy scenario?`;
  }

  if (q.includes('hello') || q.includes('hi') || q.includes('hey')) {
    return `Hello! I'm **Pharmassist AI**, your clinical pharmacy assistant at Korle Bu Teaching Hospital.

I can help you with:
- 💊 Drug interactions and contraindications
- 📋 Dosing protocols and frequency references
- 🏥 Dispensing guidelines and ward sweep protocols
- 📦 Inventory and reorder level guidance
- 🩺 Prescription interpretation and stop order procedures
- 🔬 Drug information and patient counselling points

What can I help you with today?`;
  }

  if (q.includes('thank')) {
    return `You're welcome! If you have any other clinical or pharmacy queries, feel free to ask. I'm here to support safe and efficient medication management at Korle Bu.`;
  }

  if (q.includes('furosemide') || q.includes('diuretic')) {
    return `**Furosemide (Loop Diuretic) — Key Points**

**Indications:** Heart failure, oedema, hypertension, renal impairment with fluid overload.

**Dosing:**
- Oral: 20–80mg OD or BD (titrate to response)
- IV: 20–40mg slowly over 1–2 min; higher doses in severe renal impairment

**Monitoring parameters:**
- Electrolytes (K⁺, Na⁺, Mg²⁺) — hypokalaemia is common
- Creatinine and urine output
- Blood pressure — orthostatic hypotension risk, especially in elderly

**Key interactions:**
- NSAIDs reduce diuretic efficacy
- ACE inhibitors + furosemide → first-dose hypotension
- Digoxin toxicity risk if hypokalaemia develops
- Aminoglycosides → additive ototoxicity at high IV doses

**Patient education:**
- Take in the morning to avoid nocturia
- Report muscle cramps, dizziness, or reduced urine output
- Monitor weight daily — report >2kg gain in 24h`;
  }

  if (q.includes('paracetamol') || q.includes('acetaminophen')) {
    return `**Paracetamol — Dispensing & Safety Notes**

**Standard dosing:**
- Adults: 500mg–1g every 4–6 hours, max **4g/day**
- Reduce to max 2g/day in: hepatic impairment, alcoholism, malnutrition, or low body weight (<50kg)

**Overdose risk:**
Paracetamol poisoning is the leading cause of acute liver failure in Ghana. The therapeutic window narrows significantly in patients with:
- Chronic alcohol use
- Malnutrition/low glutathione stores
- Enzyme-inducing drugs (rifampicin, phenytoin, carbamazepine)

**Key check before dispensing:**
Confirm the patient is not already receiving a combination product containing paracetamol (co-codamol, cold/flu remedies) that would push total daily dose over 4g.

**Overdose antidote:** N-acetylcysteine (NAC) — effective if given within 10–12 hours of ingestion.`;
  }

  // Generic fallback
  const responses = [
    `I don't have a specific protocol stored for that query, but I can help you research it.

**Suggested steps:**
1. Check the hospital formulary for first-line recommendations.
2. Consult the BNF (British National Formulary) or WHO Essential Medicines List for evidence-based guidance.
3. If this is an urgent clinical question, contact the senior pharmacist on duty.

Could you rephrase your question or give me more detail? For example, the drug name, patient condition, or the specific clinical scenario you're working through.`,
    `That's a good question. Let me share what I know:

For detailed queries like this, I recommend cross-referencing with:
- **Hospital formulary** — approved drug list and protocols
- **BNF or Martindale** — comprehensive drug information
- **Patient's current medication list** — for interaction screening

Please give me more specifics about the drug, patient profile, or clinical situation and I'll provide more targeted guidance.`,
  ];
